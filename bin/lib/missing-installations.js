...
const pms = [
  {
    ...
    getRequirements: lines => lines.reduce((installations, line) => {
  ...
  },
  {
    ...
    list: 'npm -g list',
    installWith: 'npm i -g',
    getRequirements: lines => lines.reduce((installations, line) => installations.concat(line.split(' ').filter(word => !~['npm', 'i', '-g'].indexOf(word))), [])
  },
  {
    name: 'apm',
    list: 'apm list',
    installWith: 'apm install',
    getRequirements: lines => lines.reduce((installations, line) => installations.concat(line.split(' ').filter(word => !~['apm', 'install'].indexOf(word))), [])
  }
]
